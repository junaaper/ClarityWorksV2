...
- The defence presentation is now served by the frontend at `/presentation`: the standalone deck and required screenshots are available from Vite public assets, and a full-screen top-level React route embeds `/presentation.html` outside the normal app layout.
- Simplify review evidence alignment now only creates word-replacement evidence from small local diff windows, suppresses protected/proper-name/title tokens such as Kafka and Gregor Samsa, carries optional original/preview evidence anchors, and uses those anchors on the frontend so misleading paragraph-wide pairs like `familial -> Samsa` or `financial -> nature` are hidden instead of highlighted.
- Low-grade long-document downgrades are no longer routed through Utrecht-only defence fallback behavior: paragraph far misses on non-defence texts can use a bounded neutral low-grade target-contract rescue, target-band Grade 3-7 paragraph rewrites survive soft noun-drop warnings when they are safer than rule fallback, and repeated title/name wording such as book titles is no longer treated as garbled text.
- Grade 12 near-downgrade finalization now exits faster after an exact clean target hit: small-shift route polish is skipped when the delivered text is already in band with no severe sanity issues, long Grade 11/12 exact hits can use one broad exact review patch instead of expensive display-evidence scanning, and authoring progress now shows candidate building, target-fit checking, final selection, and preview finalization.
- Simplify progress copy was cleaned up for defence demos: large-route internals no longer surface as `large shift llm`, paragraph progress stays at a plain `Rewriting paragraphs 2-N` message, hard Grade 5-7 defence downgrades skip confusing paragraph-repair progress and hand far misses to the defence fallback sooner.
- Defence-demo motivation-letter downgrades now fail fast when paragraph Fireworks calls stall: paragraph rewrites use bounded parallel batches and per-call timeouts, far-off paragraph-pipeline results can hand off to the defence target fallback, and body-only Utrecht letters qualify without inventing missing name/student-number headers.
- RAG source relevance now keeps FlashRank's raw cross-encoder score for diagnostics while exposing a sigmoid-plus-query-normalized relevance score for display labels and exports, preventing tiny raw rerank logits from showing as misleading 1% relevance.
- Test scripts now live under the top-level `testing/` folder: ML-service tests and utilities moved to `testing/ml-service/`, Selenium tests moved to `testing/selenium/`, path bootstrapping was updated for the new layout, and docs/command references were repointed to the new locations.
- `models.simplifier` is now a compatibility facade: `TextSimplifier` and exported constants still import from `models.simplifier`, while the implementation is split into focused `models/simplifier_parts/` mixins for routing, authoring, selection, guardrails, rules, sentence operations, diffing, paragraph rewriting, and LLM fallback.
...
- `testing/ml-service/test_hard_downgrade_selection.py` now covers target-lock near-band preference, low-grade undershoot repair by sentence combining, and meaningful reason/evidence coverage for exact broad patches.
...
- A delivery guard now forces exact broad-patch delivery when the selected candidate is near target but the finalized preview drifts back far away, preserving auto/interactive accept-all parity for broad hard-downgrade rewrites; added `testing/ml-service/test_hard_downgrade_selection.py` for Grade 16 -> 5 style selector rescue, protected-term blocking, and exact broad-patch parity.
...
- Added `testing/ml-service/test_pdf_extraction_quality.py` to cover common missing-`ti` PDF glyph repair, degraded glyph warnings, and keyword retrieval preference for explicit SDLC chunks.
...
- Added `testing/ml-service/test_simplification_golden.py` for the current worst buckets (`12 -> 3`, `11 -> 3`, `10 -> 3`, `7 -> 3`, `3 -> 9`, `4 -> 11/12/13`) plus guardrail checks for banned substitutions and phrasal-verb preservation.
- `testing/ml-service/test_simplification_matrix.py` now outputs richer diagnostics including `direction_hit`, `invalid_sentence_count`, `semantic_similarity_score`, and `reason_coverage_rate` in the per-case, per-target, and per-source summaries.
...
- Local verification for this wave: `testing/ml-service/test_simplification_consistency.py` passes, `testing/ml-service/test_simplification_golden.py` passes, Python files compile via `py_compile`, frontend `tsc --noEmit` passes, and targeted interactive/auto checks show valid outputs for `12 -> 3`, `11 -> 3`, `10 -> 3`, `7 -> 3`, `5 -> 6`, `12 -> 6`, `3 -> 9`, and `4 -> 11/12/13`.
...
- Local rewrite verification after these changes: `testing/ml-service/test_simplification_consistency.py` passes, targeted interactive/auto spot checks for `5 -> 6`, `3 -> 10`, `12 -> 6`, `12 -> 3`, and `9 -> 3` all produce valid outputs, and the full interactive matrix improved from **26/121** exact hits to **43/121** with average miss distance down to **1.87**.
- Added `testing/ml-service/test_simplification_consistency.py` to verify that interactive previews, accepted-change application, span anchors, and repeated-word partial acceptance all stay in sync.
...
- Added `testing/ml-service/test_simplification_matrix.py` to run full grade-to-grade rewrite regression checks across the calibrated test set.
